...
test('POST /entities/versions', async function ({ components }) {
  ...
  it('should return active entity correctly', async function () {
    ...
    const response = await fetchLocally('POST', '/entities/versions', undefined, { pointers: [registry.pointers[0]] })
  ...
  it('should return 200 ok and empty array when entities are not found for given pointers', async function () {
    const response = await fetchLocally('POST', '/entities/versions', undefined, { pointers: ['1000,1000'] })
    const parsedResponse = await response.json()
  ...
  it('should return single entity when multiple pointers are provided but are related to the same entity', async function () {
    ...
    const response = await fetchLocally('POST', '/entities/versions', undefined, {
  ...
  it('should return 400 when no pointers are provided', async function () {
    const response = await fetchLocally('POST', '/entities/versions', undefined, { pointers: [] })
  ...
  it('should return 400 when pointers is null', async function () {
    const response = await fetchLocally('POST', '/entities/versions', undefined, { pointers: null })
    expect(response.status).toBe(400)
    const parsedResponse = await response.json()
    expect(parsedResponse).toMatchObject({
      ok: false,
