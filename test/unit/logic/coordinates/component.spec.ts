...
import { SpawnRecalculationWithBoundsParams } from '../../../../src/types'
...
describe('when using the coordinates component', () => {
  ...
  describe('and calling recalculateSpawnIfNeeded', () => {
    let capturedCallback: ((params: SpawnRecalculationWithBoundsParams) => any) | null

    beforeEach(() => {
      capturedCallback = null
      db.recalculateSpawnCoordinate.mockImplementation(async (_worldName, callback) => {
    ...
    it('should call recalculateSpawnCoordinate with the normalized world name', async () => {
      await component.recalculateSpawnIfNeeded('Test-World')

      expect(db.recalculateSpawnCoordinate).toHaveBeenCalledWith('test-world', expect.any(Function))
    })
  ...
  describe('and calling setUserSpawnCoordinate', () => {
    describe('and the coordinate is within the world bounds', () => {
      ...
      it('should call setSpawnCoordinate with isUserSet true', async () => {
        await component.setUserSpawnCoordinate('test-world', { x: 1, y: 0 })

        expect(db.setSpawnCoordinate).toHaveBeenCalledWith('test-world', 1, 0, true)
      })
    })
    ...
    describe('and the coordinate is not within the world bounds', () => {
      ...
      it('should still call setSpawnCoordinate with isUserSet true', async () => {
        await component.setUserSpawnCoordinate('test-world', { x: 99, y: 99 })

        expect(db.setSpawnCoordinate).toHaveBeenCalledWith('test-world', 99, 99, true)
      })
    })
    ...
    describe('and the world has no processed scenes', () => {
      ...
      it('should still call setSpawnCoordinate with isUserSet true', async () => {
        ...
        expect(db.setSpawnCoordinate).toHaveBeenCalledWith('test-world', 5, 10, true)
      })
    })
  })
